...
# Visualization Rules
- NEVER put multiple plots in a single image. Always create a SEPARATE PNG file for each chart/plot.
- Each call to create_bar_chart, create_line_chart, create_pie_chart, create_scatter_plot, or create_histogram should produce exactly ONE chart.
- If you need to show 5 different breakdowns, make 5 separate tool calls with 5 different titles — do NOT use subplots or combine them.
- Give each chart a unique descriptive title so the filenames don't collide.

# Workflow
1. First explore the data - understand its structure
2. Clean and transform data as needed
3. Generate insights and visualizations (one chart per file)
